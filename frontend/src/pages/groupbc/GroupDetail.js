...
export default function GroupDetail({ match }) {
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/?book_id=${match.params.id}${userSorting}`,
                headers
            )
            .then((response) => {
    ...
    const handleDelete = () => {
        ...
        checked.forEach(function (data, idx) {
            ...
            axios
                .get(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/?book_id=${data.book_id}`, headers)
                .then((response) => {
                    response.data.forEach(function (data2, idx) {
                        if (data.id === data2.id) {
                            axios
                                .put(
                                    `http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/${data2.id}/`,
    ...
    const handleBookDelete = () => {
        ...
        axios
            .delete(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscardbook/${match.params.id}/`, headers)
            .then((response) => {
                history.push({
                    pathname: "/MyBc",
                    state: { FooterValue: 1 },
