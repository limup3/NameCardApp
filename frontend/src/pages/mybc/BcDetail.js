...
export default function BcDetail({ match }) {
    ...
    useEffect(() => {
        ...
        axios
            .get(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/${match.params.id}/`, headers)
            .then((response) => {
                ...
                if (response.data.ocr_id !== null) {
                    axios
                        .get(
                            //이미지 가져오기
                            `http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscardocr/${response.data.ocr_id}/`,
                            headers
                        )
                        .then((ocrResponse) => {
                ...
                axios
                    .put(
                        //최근 조회 업데이트
                        `http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/${match.params.id}/`,
                        {
    ...
    const handleDelete = () => {
        ...
        axios
            .delete(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/${bcItem.id}/`, headers)
            .then((response) => {
                history.push("/Mybc");
            })
            .catch((error) => {
        ...
        <>
            <Grid container>
                ...
                {ocrImage !== "" && ocrImage !== null && (
                    <img
                        style={{ marginLeft: "auto", marginRight: "auto", width: "350px", height: "300px" }}
                        src={ocrImage}
                        alt={"명함사진"}
                    />
                )}
