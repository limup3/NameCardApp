...
export default function EnteringEdit({ match }) {
    ...
    useEffect(() => {
        ...
        axios
            .get(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/?ocr_id=${match.params.id}`, headers)
            .then((response) => {
                //bccard 테이블에서 ocrid가 일치하는 데이터 찾기
                axios
                    .get(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/${response.data[0].id}/`, headers)
                    .then((response) => {
                        //찾았으면 데이터 가져오기
    ...
    const handleComplete = () => {
        ...
        axios
            .put(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscard/${id}/`, bcJson, headers)
            .then((response) => {
                axios
                    .patch(
                        `http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscardocr/${match.params.id}/`,
                        ocrJson,
                        headers
                    )
                    .then((response) => {
    ...
    const handleDelete = () => {
        ...
        axios
            .delete(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/businesscardocr/${match.params.id}/`, headers)
            .then((response) => {
                history.push("/Mybc");
            })
            .catch((error) => {
