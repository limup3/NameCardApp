...
const ChangePw = () => {
    ...
    const handleChangePw = (e) => {
        if (password === passwordConfirm) {
            ...
            //토큰 헤더로 보내기
            axios
                .post(`http://businesscard.koreacentral.cloudapp.azure.com:8000/api/rest-auth/password/change/`, userJson, headers)
                .then((response) => {
                    alert("비밀번호 재설정 완료 !");
                    // history.push({
                    //   pathname:'/MyBc',
                ...
                .catch((error) => {
                    if (error.response.data.new_password1 !== undefined) {
                        setValidPassword(error.response.data.new_password1);
                    } else {
                        setValidPassword("");
                    }
                    if (error.response.data.new_password2 !== undefined) {
                        setValidPasswordConfirm(error.response.data.new_password2);
                    } else {
